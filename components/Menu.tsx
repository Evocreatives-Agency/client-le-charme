...
const dinner: Sec[] = [
  ...
  { title: "Mains", items: [
    ...
    { name: "Flank Steak",               desc: "Boursin pomme pur\u00e9e, garlic mushrooms, bordelaise (+$3 blue cheese)", price: "40" },
  ]},
...
const brunch: Sec[] = [
  { title: "Brunch \u2014 Sat & Sun 10am\u20132pm", items: [
    ...
    { name: "French Omelette",   desc: "Fine herbs, ch\u00e8vre, roasted cherry tomatoes" },
    { name: "Eggs Benedict",     desc: "House-cured salmon, hollandaise, spinach, brioche" },
    ...
    <section id="menu" className="bg-creme py-24 md:py-32">
      <div className="max-w-5xl mx-auto px-6 md:px-10">
        ...
        <motion.div initial={{opacity:0,y:20}} whileInView={{opacity:1,y:0}} viewport={{once:true}} transition={{duration:0.6}}
          ...
          <h2 className="font-display text-[clamp(40px,5vw,64px)] italic font-normal text-encre">Our Menu</h2>
        </motion.div>

        {/* Tab selector */}
        <div className="flex gap-1 mb-12 bg-creme-dark inline-flex p-1 rounded-none">
          {(["dinner","brunch"] as const).map(t=>(
            <button key={t} onClick={()=>setTab(t)}
              className={`font-mono text-[11px] tracking-[0.18em] uppercase px-8 py-3 transition-all ${
                tab===t ? "bg-rouge text-white" : "text-encre/40 hover:text-encre/70"
              }`}>
              {t===" dinner"?"Dinner":t.charAt(0).toUpperCase()+t.slice(1)}
            </button>
          ))}
        </div>
        ...
        <AnimatePresence mode="wait">
          <motion.div key={tab} initial={{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-8}} transition={{duration:0.3}}>
            {secs.map((sec,si) => (
              <div key={sec.title} className={si>0?"mt-12 pt-12 border-t border-encre/[0.07]":""}>
                {/* Section title bar */}
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-1.5 h-6 bg-rouge flex-shrink-0" />
                  <h3 className="font-mono text-[12px] tracking-[0.22em] uppercase text-encre/60">{sec.title}</h3>
                </div>

                <div className="grid sm:grid-cols-2 gap-x-8 gap-y-0">
                  {sec.items.map(item=>(
                    <div key={item.name} className="py-4 border-b border-encre/[0.06] flex justify-between items-start gap-3">
                      <div className="flex-1 min-w-0">
                        <p className="font-mono text-[12px] font-medium tracking-[0.06em] uppercase text-rouge leading-tight mb-1">{item.name}</p>
                        {item.desc && <p className="font-body text-[14px] text-encre/45 leading-snug">{item.desc}</p>}
                      </div>
                      {item.price && <span className="font-mono text-[13px] text-encre/50 flex-shrink-0">${item.price}</span>}
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="mt-12 pt-8 border-t border-encre/[0.07] flex flex-wrap gap-4">
              <a href="https://static1.squarespace.com/static/6843723f36011b46b7924fc3/t/69a786692b5f6176a0b90965/1772586601864/2.png"
                ...
                className="inline-flex items-center gap-2 font-mono text-[11px] tracking-[0.18em] uppercase text-rouge border border-rouge/30 px-6 py-3 hover:bg-rouge hover:text-white transition-all">
