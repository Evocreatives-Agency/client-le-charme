          ...
          {(["dinner", "brunch"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`font-mono text-[11px] tracking-[0.18em] uppercase px-8 py-3 transition-all ${
                tab === t ? "bg-rouge text-white" : "text-encre/40 hover:text-encre/70 border border-encre/10"
              }`}>
              {t === "dinner" ? "Dinner" : "Brunch"}
            </button>
          ))}
        </div>
            ...
            {secs.map((sec, si) => (
              <div key={sec.title} className={si > 0 ? "mt-12 pt-12 border-t border-encre/[0.07]" : ""}>
                <div className="flex items-center gap-4 mb-6">
                  <div className="w-1 h-5 bg-rouge flex-shrink-0" />
                  <h3 className="font-mono text-[11px] tracking-[0.22em] uppercase text-encre/50">{sec.title}</h3>
                </div>
                <div className="grid sm:grid-cols-2 gap-x-8">
                  {sec.items.map(item => (
                    <div key={item.name} className="py-4 border-b border-encre/[0.06] flex justify-between items-start gap-3">
                      <div className="flex-1 min-w-0">
                        <p className="font-mono text-[11px] font-medium tracking-[0.08em] uppercase text-rouge leading-tight mb-1">{item.name}</p>
                        {item.desc && <p className="font-body text-[14px] text-encre/45 leading-snug">{item.desc}</p>}
                      </div>
                      {item.price && <span className="font-mono text-[13px] text-encre/40 flex-shrink-0">${item.price}</span>}
                    </div>
                  ))}
                </div>
              </div>
